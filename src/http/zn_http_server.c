...
typedef struct{
	...
	ZN_HttpRouter	* get_routes;
	ZN_HttpRouter	* post_routes;
}ZN_HttpServerData;
...
bool ZN_HttpServer_GestMessageDefault(ZN_TcpServer * tcp_server,ZN_TcpServerClient * socket_client, uint8_t *data, size_t data_len, void * user_data){
	...
	ZN_HttpHandleConnection * hc = ZN_HttpHandleConnection_New(socket_client, http_server,data, data_len);
	ZN_HttpHandleConnection_DoHandle(hc);
	return true;
}
...
ZN_HttpServer * ZN_HttpServer_New(
	...
	data->get_routes=ZN_HttpRouter_New();
	data->post_routes=ZN_HttpRouter_New();
